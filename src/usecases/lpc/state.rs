//! The Limitation of Power Consumption state machine, for the Controllable System.
//!
//! This is the logic a heat pump, wallbox, battery or energy manager runs when a grid
//! operator's control box limits it under §14a EnWG. It is safety-relevant in both
//! directions: too permissive and the low-voltage grid is at risk, too strict and a
//! building is throttled when it need not be.
//!
//! The states and transitions come from LPC UC TS 1.0.0 §2.2–2.3; the rules the 2026
//! implementation guide added — the ordering gate, the meaning of a lost connection,
//! and when a rejection is owed — are marked with their guide section. Every transition
//! carries the requirement identifier it implements, so the code can be read against
//! the specification line by line.
//!
//! ```
//! use core::time::Duration;
//! use eebus::usecases::lpc::{
//!     CsConfig, EffectiveLimit, LimitWrite, LocalDecision, LpcState, ControllableSystem,
//! };
//!
//! let mut cs = ControllableSystem::new(CsConfig::new(4_200.0, Duration::from_secs(7_200)),
//!                                      Duration::ZERO);
//!
//! // A restart starts limited by the failsafe value, not unlimited. [LPC-901]
...
/// ([LPC-913]; implementation guide §2.11).
...
pub enum EffectiveLimit {
    /// No limitation.
    None,
    /// The Energy Guard's active power consumption limit, in watts.
    Active(f64),
    /// The failsafe consumption active power limit, in watts.
    Failsafe(f64),
}

impl EffectiveLimit {
    /// The limit in watts, or [`None`] when consumption is unrestricted.
...
/// A write on the Active Power Consumption Limit.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct LimitWrite {
    /// Whether the limit is in force ([LPC-008]).
    pub is_active: bool,
    /// The limit in watts. Always at or above zero ([LPC-001]).
    pub watts: f64,
    /// How long the limit is valid ([LPC-004]).
    ///
    /// The duration starts running when the write arrives, whether or not the limit is
    /// active, and the limit is deactivated when it reaches zero ([LPC-007]).
...
/// Why a Controllable System may refuse a limit (LPC UC TS §2.2).
...
pub enum WriteOutcome {
    /// Accepted: `ACK`, [LPC-002].
    Accepted,
    /// Refused: `NACK`, [LPC-003].
    Rejected(NackReason),
}
...
pub enum NackReason {
    ...
    /// A negative limit, which [LPC-001] and implementation guide §3.6 forbid.
    #[error("a limit below zero is not permitted")]
    NegativeValue,
    /// A failsafe duration outside the two-to-twenty-four-hour range ([LPC-022/4]).
...
pub struct CsConfig {
    ...
    /// The Failsafe Duration Minimum ([LPC-022]), between two and twenty-four hours.
    pub failsafe_duration: Duration,
    /// The largest Failsafe Duration Minimum this device accepts ([LPC-022/4]).
    pub failsafe_duration_max: Duration,
    /// The nominal maximum power the device can consume ([LPC-041]), if it is a device
    /// rather than an energy manager.
    pub nominal_max_watts: Option<f64>,
    /// The contractual maximum an energy manager is allowed to consume ([LPC-042]).
...
impl CsConfig {
    ...
    /// Sets the device's nominal maximum consumption ([LPC-041]).
    #[must_use]
    ...
    /// Sets the contractual maximum consumption of an energy manager ([LPC-042]).
    #[must_use]
...
/// The Controllable System actor of Limitation of Power Consumption.
#[derive(Clone, Debug)]
pub struct ControllableSystem {
    ...
    ///
    /// [LPC-921] measures its window from that heartbeat, not from entering the state,
...
impl ControllableSystem {
    /// Starts in `init`, limited by the failsafe value ([LPC-901]).
    ...
    pub fn effective_limit(&self) -> EffectiveLimit {
        match self.state {
            LpcState::Init | LpcState::FailsafeState => {
                EffectiveLimit::Failsafe(self.config.failsafe_watts)
            }
            LpcState::Limited => match &self.limit {
                Some(limit) if limit.is_active => EffectiveLimit::Active(limit.watts),
                _ => EffectiveLimit::None,
            },
            LpcState::UnlimitedControlled | LpcState::UnlimitedAutonomous => EffectiveLimit::None,
        }
    }

    /// Whether the Active Power Consumption Limit is currently activated, which is what
    /// `loadControlLimitData.isLimitActive` reports ([LPC-009]).
    pub fn is_limit_active(&self) -> bool {
        self.state == LpcState::Limited && self.limit.is_some_and(|l| l.is_active)
    }

    /// Records a heartbeat from the Energy Guard ([LPC-031]).
    pub fn on_heartbeat(&mut self, now: Duration) {
        self.last_heartbeat = Some(now);
        if self.state == LpcState::FailsafeState && self.awaiting_write_since.is_none() {
            self.awaiting_write_since = Some(now);
        }
    }

    /// Applies a write on the Active Power Consumption Limit.
    ///
    ...
    pub fn on_limit_write(
        ...
        // [LPC-001], implementation guide §3.6: a limit below zero is never valid.
        if write.watts < 0.0 {
        ...
        if let LocalDecision::Reject(reason) = decision {
            // [LPC-907]: a rejection leaves the controlled states untouched. From the
            // failsafe or autonomous states the write still counts as contact, and
            // [LPC-918] moves the system to unlimited/controlled.
            match self.state {
                LpcState::Limited | LpcState::UnlimitedControlled => {}
                _ => self.enter(LpcState::UnlimitedControlled, now),
            }
        ...
        // [LPC-007] and implementation guide §2.2: a duration of zero deactivates the
        ...
        if write.is_active && !expired_immediately {
            ...
            self.enter(LpcState::Limited, now);
        } else {
            // [LPC-902], [LPC-905], [LPC-909], [LPC-920]
            self.enter(LpcState::UnlimitedControlled, now);
        }
        WriteOutcome::Accepted
    }

    /// Applies a write on the Failsafe Consumption Active Power Limit ([LPC-021]).
    ...
    /// Applies a write on the Failsafe Duration Minimum ([LPC-022]).
    ///
    /// The value must lie between two and twenty-four hours and at or below this
    /// device's own maximum; [LPC-022/5] then requires the device to report the value it
    ...
    pub fn handle_timeout(&mut self, now: Duration) {
        // [LPC-911], [LPC-912]: no heartbeat for 120 seconds, from either controlled
        ...
        if matches!(
            ...
            >= HEARTBEAT_TIMEOUT
        {
            self.enter(LpcState::FailsafeState, now);
            return;
        }

        // [LPC-908]: an expired duration deactivates the limit.
        if self.state == LpcState::Limited
        ...
        match self.state {
            // [LPC-906]: nothing heard in the settle window after a restart.
            LpcState::Init if now.saturating_sub(self.entered_state_at) >= SETTLE_TIMEOUT => {
    ...
    pub fn poll_timeout(&self) -> Option<Duration> {
        ...
        match self.state {
            ...
            LpcState::Init => consider(self.entered_state_at + SETTLE_TIMEOUT),
            LpcState::FailsafeState => {
    ...
    fn needs_recent_heartbeat(&self) -> bool {
        matches!(
            self.state,
            LpcState::Init | LpcState::FailsafeState | LpcState::UnlimitedAutonomous
        )
    }
    ...
    fn enter(&mut self, state: LpcState, now: Duration) {
        if self.state != state {
            self.state = state;
