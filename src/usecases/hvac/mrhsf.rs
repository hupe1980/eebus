//! Monitoring of Room Heating System Function (MRHSF).
//!
//! A *Monitoring Appliance* reads which operation mode a room's heating is in. It is the
//! half of the room heating pair that says **what the room is doing**;
//! [`crhsf`](super::crhsf) is the half that changes it.
//!
//! One scenario, mandatory for both actors (Table 1):
//!
//! 1. **Monitor room heating operation mode** — `auto`, `on`, `off` or `eco`, exactly one
...
pub const NAME: &str = "monitoringOfRoomHeatingSystemFunction";
...
///
/// Read-only: this use case reports, and [`crhsf`](super::crhsf) is where the mode is
...
const SCENARIO_NAME: &str = "Monitor room heating operation mode";

/// The HVAC Room: the actor whose heating is reported.
