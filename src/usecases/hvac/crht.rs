//! Configuration of Room Heating Temperature (CRHT).
//!
//! A *Configuration Appliance* sets the heating temperature setpoint of an *HVAC Room* —
//! "a logical or physical indoor space of a house or premises". It is [`cdt`](super::cdt)
//! pointed at a room instead of a tank.
//!
//! One scenario, mandatory for both actors (Table 1):
//!
//! 1. **Set room heating temperature setpoint** — the appliance writes a temperature, the
//!    room applies it within the range and step size it published [CRHT-001].
//!
...
//! function is `heating` for this use case and `cooling` for
//! [`crct`](super::crct). So which setpoint to write is only answerable once
//! [`mrhsf`](super::mrhsf) has said which mode the heating function is in —
//! [`Setpoints::for_mode`](super::setpoint::Setpoints::for_mode) takes both identifiers for
//! that reason, and a reader keyed by the mode alone would warm a room it was asked to cool.
//!
//! [CRHT-005] says the same thing: an actor here SHOULD also support "Configuration of Room
//! Heating System Function", and a Configuration Appliance that does not SHOULD support the
//! monitoring one.
//!
//! # And whether it worked
//!
//! [CRHT-004] ties the setpoint to the measurement: its `measurementId` SHALL be the one
...
//! use eebus::usecases::hvac::crht;
//!
//! let mut known = crht::reader();
//! known.learn(&crht::setpoint_description(UnitOfMeasurement::DegC));
//! known.learn(&crht::setpoint_constraints(16.0, 26.0, Some(0.5)));
//!
//! let id = known.temperature_setpoints().next().expect("a room air setpoint");
//! assert!(known.write(id, 21.0).is_ok());
//! assert!(known.write(id, 30.0).is_err());
//! ```
...
pub const NAME: &str = "configurationOfRoomHeatingTemperature";
...
///
/// **Shared with [`crct`](super::crct)**, which is why it is not enough on its
...
///
/// The same number [`mrhsf`](super::mrhsf) publishes the room heating under, which
/// [CRHT-005] requires, and *not* the one [`crct`](super::crct) uses.
...
///
/// A room that also serves [`mrhsf`](super::mrhsf) puts both on one feature:
/// [`mrhsf::with_setpoints`](super::mrhsf::with_setpoints).
...
///
/// [CRHT-004]: the `measurementId` SHALL be the one that use case publishes the room
...
/// Which setpoints each operation mode of the heating function uses (Table 10).
///
/// Published under [`SYSTEM_FUNCTION_ID`], which is what keeps these relations apart from
/// [`crct`](super::crct)'s on the same feature.
...
/// it collected is the heating one.
...
pub const SUBJECT: Subject = Subject::temperature(HEATING, TEMPERATURE_SCOPE);
...
///
/// Two features, not one: the setpoints live on `Setpoint` and the relation that says
/// which mode uses which setpoint lives on `HVAC`. Both are located and both are
/// subscribed to.
///
...
/// and which mode the room is in is [`mrhsf`](super::mrhsf)'s or [`crhsf`](super::crhsf)'s
/// to say — which is why [CRHT-005] makes one of those two mandatory for a room serving this.
/// Join the two readers with
...
const SCENARIO_NAME: &str = "Set room heating temperature setpoint";
...
mod tests {
    ...
    fn a_value_the_room_would_refuse_is_refused_here() {
        ...
        known.learn(&setpoint_constraints(16.0, 26.0, Some(0.5)));
    ...
    /// The gate: a write into a mode the room's heating is not in changes nothing.
    #[test]
    fn a_write_into_the_wrong_mode_is_refused() {
        let auto = mrhsf::operation_mode_id(&HvacOperationModeType::Auto).unwrap();
        let off = mrhsf::operation_mode_id(&HvacOperationModeType::Off).unwrap();
        ...
        let mut state = mrhsf::reader();
        state.learn(&mrhsf::system_function_description());
        state.learn(&mrhsf::operation_mode_descriptions(&modes).unwrap());
        state.learn(&mrhsf::operation_mode_relations(&modes).unwrap());

        state.learn(&mrhsf::system_function_state(off, false, None));
        assert_eq!(
            known.effect_of(SETPOINT_ID, &state),
            SetpointEffect::NotInCurrentMode
        );
        assert!(matches!(
            known.write_effective(SETPOINT_ID, 21.0, &state),
            Err(WriteRefused::NotInCurrentMode)
        ));

        state.learn(&mrhsf::system_function_state(auto, false, None));
        assert_eq!(
            known.effect_of(SETPOINT_ID, &state),
            SetpointEffect::Effective
        );
        assert!(known.write_effective(SETPOINT_ID, 21.0, &state).is_ok());
    }
