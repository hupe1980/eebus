...
//!
//! It is the EV counterpart of [LPC](crate::usecases::lpc), and the differences are what
//! makes it a separate use case rather than a parameter:
//!
//! | | LPC | OPEV |
//! |---|---|---|
//! | What is limited | total active power, in watts | charging current, **per phase**, in amperes |
//! | Heartbeat timeout | 120 s | **4 s** ([OPEV-005]) |
...
//! use eebus::usecases::emobility::opev::{ChargingCurrents, EvCharging};
//!
...
const GUARD_WRITES: &[FunctionUse] = &[
    FunctionUse::client(
        FeatureType::LoadControl,
        Function::LoadControlLimitDescriptionListData,
    ),
...
const GUARD_SERVES: &[FunctionUse] = &[
    FunctionUse::server(
        FeatureType::DeviceDiagnosis,
        Function::DeviceDiagnosisHeartbeatData,
    ),
...
///
/// `maxValueLimit` and `overloadProtection`, in amperes: this is a ceiling on current for
/// the sake of a fuse, which is what tells the Energy Guard the number it writes is not a
/// power and not a target.
pub fn limit_descriptions(phases: &[Phase]) -> CmdData {
            ...
            phases
                ...
                .filter_map(|phase| {
                    ...
                    Some(LoadControlLimitDescriptionData {
                        ...
                        scope_type: Some(ScopeType::OverloadProtection),
                        ..Default::default()
                    })
                })
                .collect(),
...
mod tests {
    ...
    fn the_descriptors_say_what_the_specification_says() {
        ...
        assert_eq!(EV.required_scenarios().collect::<Vec<_>>(), [1, 2, 3]);
        assert_eq!(
        ...
        assert_eq!(
            ENERGY_GUARD.features_needing_binding().collect::<Vec<_>>(),
            [&FeatureType::LoadControl]
        );
    }
    ...
    fn the_limit_description_says_amperes_and_overload_protection() {
        let CmdData::LoadControlLimitDescriptionListData(list) = limit_descriptions(&PHASES) else {
...
impl EvActor {
    ...
    pub fn publish(&self, engine: &mut Engine, now: Duration) {
        ...
        if let Some(feature) = engine.device_mut().resolve_mut(&load_control) {
            let _ = feature.set_data(limit_descriptions(&self.phases));
...
pub fn locate_guard(remote: &RemoteDevice) -> Option<FeatureAddress> {
    let use_case = ENERGY_GUARD_ACTORS
        .iter()
        .find_map(|actor| remote.use_case(names::OPEV, actor))?;
    remote.address_of(use_case, &FeatureType::DeviceDiagnosis, Role::Server)
}

/// Finds a car's OPEV features from its discovery data.
pub fn locate(remote: &RemoteDevice) -> Option<EvPeer> {
    let use_case = remote.use_case(names::OPEV, actors::EV)?;
    Some(EvPeer {
