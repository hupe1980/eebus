//! The Controllable System state machine of Limitation of Power Consumption/Production.
//!
...
//!
//! They cover LPP at the same time. LPP UC TS §2.3.3 states the same thirteen
//! transitions with the same identifiers under an `LPP` prefix, which is why one state
//! machine serves both; `limitation_over_the_wire.rs` checks the part that does differ.
...
use eebus::usecases::limitation::{
    ...
    NackReason, RejectReason, WriteOutcome,
};
...
fn lpc_906_init_to_unlimited_autonomous_after_120_seconds() {
    ...
    assert_eq!(cs.state(), LimitationState::Init, "not yet");
...
fn lpc_918_919_920_leaving_the_failsafe_state() {
    for (write, expected) in [
        (LimitWrite::active(2_500.0), LimitationState::Limited),
        (
